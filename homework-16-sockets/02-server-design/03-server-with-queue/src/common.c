...
int send_message_to(int fd, const char *message, const struct sockaddr_in *addr, socklen_t addr_len) {
    int bytes_sent = sendto(fd, message, strlen(message), 0, (struct sockaddr*)addr, addr_len);
    if (bytes_sent != -1) {
...
int receive_message_from(int fd, struct sockaddr_in *addr, socklen_t *addr_len) {
    ...
    int bytes_read = recvfrom(fd, buf, BUF_LEN - 1, 0, (struct sockaddr*)addr, addr_len);
    if (bytes_read != -1) {
        buf[bytes_read] = '\0';
